...
description: >
  ...
  implementation starts. Runs 11 deterministic structural checks on a feature's
  ...
  (check #10), and the spec's status agrees with its roadmap feature's status
  (check #11: BLOCKER on a terminal disagreement, i.e. one side `done` or
  `cancelled`; a non-terminal difference only warns). Checks #9/#10 are the
  spec↔plan consistency gate (the former a1-check skill, retired M13) —
  a1-new-feature's Phase 4.5 runs them together with #11 via `--only 9,10,11`
  (exit 0/1/2). Severity maps to BLOCKER (stops the
  ...
  (alias: "konsistenz-check für <feature>"), "fr-coverage check", "can I start
  implementation now" (alias: "kann ich jetzt mit der umsetzung starten"),
  "check wave-plan completeness" (alias: "prüf die
  wave-plan-vollständigkeit"), "checklist run <slug>", "a1-checklist", or any
  request to verify whether a wave-plan is ready to be handed to executing
  agents. Exit semantics: 0=PASS or PASS_WITH_WARNINGS, 1=FAIL (BLOCKER),
  2=ERROR (setup). Do NOT activate for: semantic spec review
...
translates the structured result into user-facing guidance (user's language —
see `_shared/language-policy.md`).
