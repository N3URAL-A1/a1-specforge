...
`--vault <path>`), forward it. Default is the repo-local `.a1/learnings/` store.
...
| `wave_plan_exists` | "There is no wave plan under `project/<slug>/plans/`. Should I trigger `a1-vincente-vibe-optimizer` to create the plan — or start `a1-new-feature` Phase 4?" |
