...
// branch lazily requires it and calls product.init({ usage, CODE_SCOPE_STAGES }) once
// before dispatching (usage() depends on the facade-only HELP const; CODE_SCOPE_STAGES
// is shared with the facade-resident code-scope commands — neither can move without
// creating a circular require back into this file).
...
function main() {
  ...
  try {
    if (group === 'spec') {
      ...
      // one-time init() to inject the two facade-only free identifiers the
      // moved functions still call (usage() depends on HELP; CODE_SCOPE_STAGES
      // is shared with the facade-resident code-scope commands).
